...
// A real row captured from api2thread/list, trimmed of unused fields but
// keeping every index the parser reads.
function rawRow(overrides: { direction?: string; tmpId?: string | null } = {}): unknown[] {
  const row = new Array(30).fill(null);
  row[0] = "79be97674bea1ae1f0cc2bb88cd71a1bc30e12be";
  row[1] = 1788212496449;
  row[2] = "+12143027963";
  row[9] = overrides.direction ?? "RECEIVE MESSAGE";
  row[15] = "+14697590653";
...
// array indices, mixing up who sent what or which thread it belongs to.
describe("parseThreadEvent", () => {
  test("decodes id, timestamp, parties, direction, and thread id", () => {
  ...
  test("carries a tmpId when present, for events this client sent", () => {
    const event = parseThreadEvent(rawRow({ direction: "SEND MESSAGE", tmpId: "237249786866331" }));
    expect(event.direction).toBe("SEND MESSAGE");
    expect(event.tmpId).toBe("237249786866331");
  });

  // If this fails: an unrecognized direction label is silently accepted
  // instead of surfacing that the API's wire format changed.
  test("throws on an unrecognized direction label", () => {
    expect(() => parseThreadEvent(rawRow({ direction: "CALL" }))).toThrow(/Unrecognized/);
  });
});
...
describe("parseThreadListResponse", () => {
  test("decodes threads and their events from the wrapped response body", () => {
    const body = [
      [
        ["t.+14697590653", 0, [rawRow(), rawRow({ direction: "SEND MESSAGE", tmpId: "1" })]],
      ],
      "1",
      "v",
    ];
    ...
    expect(threads[0]!.events[1]!.direction).toBe("SEND MESSAGE");
  });
});
